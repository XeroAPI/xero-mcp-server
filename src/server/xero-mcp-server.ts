...
const xeroMcpInstructions = [
  ...
  "First call prepare-file-upload, then Cowork uploads the selected local file to the returned uploadUrl using multipart form data, then call add-attachment or upload-file with stagedFileId. The preferred uploadUrl uses the same /mcp connector endpoint with a stagedUploadId query parameter so host sandboxes can treat it as connector traffic.",
