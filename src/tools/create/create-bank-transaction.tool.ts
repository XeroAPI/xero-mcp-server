...
const CreateBankTransactionTool = CreateXeroTool(
  ...
  async ({ type, bankAccountID, contactID, lineItems, reference }) => {
    const result = await createXeroBankTransaction(type, bankAccountID, contactID, lineItems, reference);
    ...
    const deepLink = bankTransaction.bankAccount.accountID && bankTransaction.bankTransactionID
      ? await bankTransactionDeepLink(bankTransaction.bankAccount.accountID, bankTransaction.bankTransactionID)
      : null;
